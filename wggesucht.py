def main():
    ...
    if selected == "🏘️ Apartments":
        ...
        with col2:
            ...
            st.write(table_df["EUR / SQM"].min())
            st.write(table_df["EUR / SQM"].max())
            slider_europersqm = st.slider("Chosse an euro per sqm range", int(table_df["EUR / SQM"].min()), int(table_df["EUR / SQM"].max()), (0, table_df["EUR / SQM"].max))
