    ...
    if selected == "🏘️ Apartments":
        ...
        with col3:
            ...
            chart = alt.Chart(df_concat_pivot_releasedate).encode(
                ...
                y=alt.Y('Eintrag:N', sort=None, axis=alt.Axis(title='Entry date')), #use 'sort=None' to preserve the order of categories
        ...
        with col1:
            ...
            chart = alt.Chart(df_concat).mark_point(color="orange").encode(
                x=alt.X('EUR / SQM:Q', title='EURO per SQM', axis=alt.Axis(tickCount=5)).scale(reverse=True),
                y=alt.Y('Lease term:Q', title='Lease term', axis=alt.Axis(tickCount=5)).scale(reverse=True),
        ...
        with col2:
            ...
            chart = alt.Chart(df_concat).mark_point(color="orange").encode(
                ...
                tooltip=['Neighbourhood', 'EUR / SQM', 'Link']
            )
            # show the chart
            st.altair_chart(chart.interactive(), use_container_width=True)
