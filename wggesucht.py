def main():
    ...
    if selected == "🏘️ Apartments":
        ...
        with col1:
            text = "In addition to its data manipulation and analysis capabilities, Streamlit also offers powerful charting functionalities. We can utilize Streamlit to create visually appealing and interactive charts, such as line plots, bar charts, scatter plots, and more. These charts enable us to visualize trends, patterns, and relationships within the data, making it easier to communicate insights effectively. Furthermore, Streamlit's web application framework allows us to automate report generation by integrating the scraped data, data transformations, analysis, and visualizations into a cohesive workflow. We can create a web application that automatically updates and presents the latest information, eliminating the need for manual report generation. Streamlit also provides features for creating user interfaces and implementing authentication mechanisms. We can build login systems and customize interfaces for different departments or users, ensuring secure access to specific data and functionalities. This level of customization and user management enhances the usability and scalability of the web application, catering to the needs of different stakeholders within an organization.\n\n By leveraging Streamlit's charting capabilities and web application framework, we can streamline the data analysis process, automate reporting, and create tailored user experiences for various departments or users within an organization."
